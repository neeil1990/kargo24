...
foreach ($arResult['SECTIONS'] as $key => $arSection){
	...
	if($key_title){
		$arResult['ITEM_SECTION'][$key_title][] = $arSection;
	}
	else
		continue;
...
if($sort)
$sort($arResult['ITEM_SECTION']);

$col = 1;
foreach($arResult['ITEM_SECTION'] as $district => $section){
	if($col > 4)
		$col = 1;

	$arResult['ITEM_SECTION_COLS'][$col][$district] = $section;
		$col++;
}
?>